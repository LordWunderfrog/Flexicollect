 ...
 *
 * This component is used to manage the client.
 * 
 */
...
class CreateClient extends Component {
    ...
    api2
      ...
      .then(resp => {
        ...
        if (String(client.id) === String(id)) {
          this.form.setFieldValue("id", client.id);
          this.form.setFieldValue("clientName", client.clientName);
          this.form.setFieldValue("clientImage", client.clientImage);
          this.form.setFieldValue("clientImageType", "");
          this.form.setFieldValue("country", client.country);
          this.form.setFieldValue("owner", client.clientOwner.name);
          this.form.setFieldValue("email", client.clientOwner.email);
          this.form.setFieldValue("mobile", client.clientOwner.mobile);
          this.form.setFieldValue("location", client.location);
          this.form.setFieldValue("tags", client.tags);
          this.form.setFieldValue("notes", client.notes);
  ...
  /* Handles the event to form the client data. */
  prepareRestData = values => {
    return {
      ...
      clientImageType: values.clientImageType,
      notes: values.notes
    };
  };
  ...
  /* Handles the api to create a new client. */
  addClient(data) {
    this.openLoading();
    api2
      .post("client", data)
  ...
  /* Handles the api to update a client. */
  updateClient(data) {
    this.openLoading();
    api2
      .patch("client", data)
  ...
  /* Handles the event to update the mode. */
  editItem = () => {
  ...
  onDrop(file) {
    this.getBase64(file).then(data => {
      ...
      if (this.isImage(fileTypeCut)) {
        this.form.setFieldValue("clientImage", data);
      } else {
        this.form.setFieldValue("clientImage", null);
      }
      this.form.setFieldValue("clientImageType", fileTypeCut);
      this.form.setFieldValue("clientImageName", file.name);
    });
  }
  ...
  render() {
    ...
    if (isCreated) {
      return <Redirect to="/home/clients" />;
    }
          ...
          <GridContainer style={{ width: "95%", height: "auto" }}>
            <Card style={{ height: "auto" }}>
              <CardHeader color="primary">
                <h4 className={classes.cardTitleWhite}>{this.state.mode === "view" ? "View Client" : "Create Client"}</h4>
              </CardHeader>
              <Formik
